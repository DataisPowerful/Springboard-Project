...
SELECT name, monthlymaintenance, CASE WHEN monthlymaintenance > 100 THEN 'expensive' ELSE 'cheap' END AS facilitydiv FROM `Facilities`;
...
SELECT DISTINCT b.name, CONCAT(c.firstname, ' ', c.surname) AS memname
FROM ((`Bookings`  a 
...
SELECT b.name, CONCAT(c.firstname,' ', c.surname) AS Mbrname, SUBSTRING(a.starttime,1,10), 
       CASE WHEN a.memid > 0 THEN a.slots * b.membercost
            ELSE a.slots * b.guestcost
       END AS cost
FROM ((`Bookings` a 
INNER JOIN `Facilities` b ON a.facid = b.facid)
INNER JOIN `Members` c ON a.memid = c.memid)
WHERE SUBSTRING(a.starttime,1,10) = '2012-09-14'
HAVING cost > 30
ORDER BY cost DESC;
